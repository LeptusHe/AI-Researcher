...
def plan_generation_method(method, idea, demo_examples, topic_description, openai_client, model, seed, client_type=None):
    ## forumate an idea from a paragraph into a full experiment plan based on our template

    prompt = "You are an expert researcher in mobile graphics and real-time rendering, and your job is to expand a brief project idea into a full project proposal with detailed experiment plans so that your students can follow the steps and execute the full project. I will provide you with an idea on the topic of: " + topic_description + ".\n\n"
    ...
    prompt += "5. Step-by-Step Experiment Plan: Break down every single step of the experiments, make sure every step is executable. Cover all essential details such as the test scenes/datasets, target devices, rendering techniques, and metrics to be used. Specify the mobile devices for testing, GPU profiling tools (e.g., Snapdragon Profiler, ARM Mali Graphics Debugger, Xcode GPU debugger), and quality/performance metrics (PSNR, SSIM, LPIPS for quality; FPS, frame time, power consumption for performance).\n"
    prompt += "6. Test Case Examples: Give two concrete examples. The first example should show how the baseline method performs on a test scenario (e.g., a specific scene on a specific mobile device). The second example should show how the proposed method improves upon the baseline on the same test scenario. For each test case, include the scene description, device, and the expected metrics. If the proposed method has multiple steps, break them down into intermediate steps.\n"
    ...
    if method == "rendering_optimization":
        prompt += "When designing experiments, focus on rendering pipeline optimizations that work within mobile GPU constraints. Assume the students have access to common mobile devices (Android phones with Adreno/Mali GPUs, iPhones with Apple GPUs) and standard GPU profiling tools, but limited access to custom hardware or large GPU clusters. Avoid experiments that require desktop-class GPU resources.\n"
    prompt += "Be consistent in your methodology and experiment design, for example, if you claim to target mobile GPUs, then all experiments should be runnable on mobile hardware, and you should not propose experiments that require desktop-class GPU resources unless explicitly for comparison purposes.\n"
...
if __name__ == "__main__":
    ...
    for idea_name in tqdm(idea_names):
        ...
        try:            
            ...
            prompt, response, cost = plan_generation_method(args.method, idea, demo_examples, topic_description, client, args.engine, args.seed, client_type=client_type)
            # print (response)
            print ("cost: ", cost)
